...
@api_router.get("/admin/prices/blast-radius")
async def admin_price_blast_radius(
    row_threshold: int = Query(50, ge=1, le=500, description="Max row_count to consider 'collapsed'"),
):
    """
    Phase C destructive-write blast-radius diagnostic.

    Scans ALL tracked tickers that have price_history_complete=True or
    have been processed by Phase C, cross-joins with live stock_prices
    row counts, and reports tickers showing the row-collapse symptom.

    Detection definition:
      A ticker is "row-collapsed" if it has <= row_threshold rows in
      stock_prices AND (price_history_complete=True OR was previously
      processed by Phase C as indicated by having ingestion_audit,
      range_proof, or history_download_proven_at fields).

    Returns the exact table requested:
      ticker | before_rows | after_rows | earliest_date | latest_date |
      audit_id/job_run_id | failure_reason | reflagged? | is_visible?
    """
    # ── Step 1: Find tickers that Phase C has touched ────────────────
    # A ticker was "Phase C processed" if it has any of these fields set.
    phase_c_touched_query = {
        "$or": [
            {"price_history_complete": True},
            {"ingestion_audit": {"$exists": True}},
            {"range_proof": {"$exists": True}},
            {"history_download_proven_at": {"$exists": True}},
            {"price_history_status": {"$exists": True}},
            {"needs_price_redownload": True},
        ]
    }
    phase_c_tickers = await db.tracked_tickers.find(
        phase_c_touched_query,
        {
            "_id": 0,
            "ticker": 1,
            ...
            "history_download_error": 1,
            "history_download_http_status": 1,
            "history_download_failed_at": 1,
            "ingestion_audit": 1,
            "range_proof": 1,
        },
    ).to_list(None)
    ...
    if not phase_c_tickers:
        return {
            "status": "no_phase_c_tickers",
            "message": "No tickers have been processed by Phase C yet.",
            "affected": [],
            "totals": {"total_affected": 0, "visible": 0, "reflagged": 0},
        }

    ticker_map = {doc["ticker"]: doc for doc in phase_c_tickers}
    all_tickers = list(ticker_map.keys())

    # ── Step 2: Get LIVE row counts from stock_prices ────────────────
    live_counts_cursor = db.stock_prices.aggregate([
    ...
    live_counts = {doc["_id"]: doc async for doc in live_counts_cursor}

    # ── Step 3: Also check tickers with ZERO rows (not in stock_prices at all)
    tickers_with_data = set(live_counts.keys())
    tickers_without_data = set(all_tickers) - tickers_with_data

    # ── Step 4: Build the affected list ──────────────────────────────
    affected = []
    for ticker in sorted(all_tickers):
        doc = ticker_map[ticker]
        live = live_counts.get(ticker)
        current_rows = live["row_count"] if live else 0
        earliest = live["earliest_date"] if live else None
        latest = live["latest_date"] if live else None

        # Only report tickers at or below the collapse threshold
        if current_rows > row_threshold:
            continue

        # Extract before_rows from ingestion_audit if available
        audit = doc.get("ingestion_audit") or {}
        before_rows = audit.get("pre_db_count")
        if before_rows is None:
            before_rows = "unknown"

        # Failure reason from tracked_tickers fields
        failure_reason = doc.get("history_download_error") or doc.get("price_history_status") or "unknown"
        http_status = doc.get("history_download_http_status")
        if http_status:
            failure_reason = f"{failure_reason} (http={http_status})"

        reflagged = bool(doc.get("needs_price_redownload"))
        is_visible = bool(doc.get("is_visible"))
        ...
        affected.append({
            ...
            "price_history_complete": bool(doc.get("price_history_complete")),
        })

    total_affected = len(affected)
    total_visible = sum(1 for a in affected if a["is_visible"])
    total_reflagged = sum(1 for a in affected if a["reflagged"])

    # ── Step 5: Generate markdown table ──────────────────────────────
    md_lines = [
        "| ticker | before_rows | after_rows | earliest_date | latest_date | failure_reason | reflagged? | is_visible? |",
        "|--------|-------------|------------|---------------|-------------|----------------|------------|-------------|",
    ]
    for a in affected:
        md_lines.append(
            f"| {a['ticker']} | {a['before_rows']} | {a['after_rows']} | "
            f"{a['earliest_date'] or 'N/A'} | {a['latest_date'] or 'N/A'} | "
            f"{a['failure_reason']} | {'✅' if a['reflagged'] else '❌'} | "
            f"{'✅' if a['is_visible'] else '❌'} |"
        )

    return {
        "status": "ok",
        "detection_definition": (
            f"Tickers processed by Phase C with <= {row_threshold} rows in stock_prices. "
            "before_rows from ingestion_audit.pre_db_count (or 'unknown' for pre-fix runs)."
        ),
        "queries_used": {
            "phase_c_touched": str(phase_c_touched_query),
            "live_row_counts": "db.stock_prices.aggregate([{$match: {ticker: {$in: [...]}}}, {$group: {_id: '$ticker', row_count: {$sum: 1}, earliest_date: {$min: '$date'}, latest_date: {$max: '$date'}}}])",
            "collapse_filter": f"current_rows <= {row_threshold}",
        },
        "affected": affected,
        "totals": {
            "total_phase_c_tickers_scanned": len(all_tickers),
            "total_affected": total_affected,
            "visible": total_visible,
            "reflagged": total_reflagged,
            "not_yet_reflagged": total_affected - total_reflagged,
        },
        "markdown_table": "\n".join(md_lines),
...
@api_router.post("/admin/prices/reflag-collapsed")
async def admin_reflag_collapsed_tickers(
    row_threshold: int = Query(50, ge=1, le=500, description="Max row_count to consider 'collapsed'"),
    dry_run: bool = Query(True, description="If true, only report — do not update"),
):
    """
    Reflag all row-collapsed tickers for safe Phase C redownload.

    Sets needs_price_redownload=True and price_history_complete=False
    on every ticker that has <= row_threshold rows in stock_prices and
    was previously processed by Phase C.

    Idempotent: safe to rerun. Only sets flags, does not delete any data.
    """
    # Same detection logic as blast-radius endpoint
    phase_c_touched_query = {
        "$or": [
            {"price_history_complete": True},
            {"ingestion_audit": {"$exists": True}},
    ...
    if not phase_c_tickers:
        return {"status": "no_work", "reflagged": 0}

    all_tickers = [doc["ticker"] for doc in phase_c_tickers]
    ticker_flags = {doc["ticker"]: bool(doc.get("needs_price_redownload")) for doc in phase_c_tickers}

    # Get live row counts
    live_counts_cursor = db.stock_prices.aggregate([
        {"$match": {"ticker": {"$in": all_tickers}}},
        {"$group": {"_id": "$ticker", "row_count": {"$sum": 1}}},
    ])
    live_counts = {doc["_id"]: doc["row_count"] async for doc in live_counts_cursor}

    # Find collapsed tickers not yet reflagged
    to_reflag = []
    already_flagged = []
    for ticker in sorted(all_tickers):
        current_rows = live_counts.get(ticker, 0)
        if current_rows > row_threshold:
            continue
        if ticker_flags.get(ticker):
            already_flagged.append(ticker)
        else:
            to_reflag.append(ticker)

    if not dry_run and to_reflag:
        ...
        await db.tracked_tickers.update_many(
            ...
            {"$set": {
                ...
                "price_history_status": "reflagged_by_blast_radius_remediation",
                "blast_radius_reflagged_at": now,
                "updated_at": now,
            }},
        )
    ...
    return {
        ...
        "newly_reflagged": to_reflag,
        "newly_reflagged_count": len(to_reflag),
        "already_flagged": already_flagged,
        "already_flagged_count": len(already_flagged),
        "total_collapsed": len(to_reflag) + len(already_flagged),
        "message": (
            f"{'Would reflag' if dry_run else 'Reflagged'} {len(to_reflag)} tickers. "
            f"{len(already_flagged)} already had needs_price_redownload=True. "
            f"Next Phase C run will restore history using fetch-before-delete. "
