...
async def admin_find_malformed_price_docs():
    """Find stock_prices documents missing required fields (date or close).

    Production query:
        db.stock_prices.aggregate([
          {$match: {$or: [{date: {$exists: false}}, {date: null},
    ...
    results = await db.stock_prices.aggregate(pipeline).to_list(length=5000)
    return {
        "total_tickers_affected": len(results),
        "tickers": [
            {
                "ticker": r["_id"],
...
async def admin_purge_malformed_price_docs(
    dry_run: bool = Query(True, description="If true, only report what would be deleted"),
):
    """Purge malformed stock_prices docs and reflag affected tickers.

    Malformed = missing ``date`` OR missing ``close``.
    ...
    # Identify affected tickers BEFORE deletion
    affected = await db.stock_prices.distinct("ticker", predicate)
    count = await db.stock_prices.count_documents(predicate)

    if dry_run:
        return {
            "dry_run": True,
            "would_delete_docs": count,
            "would_reflag_tickers": len(affected),
            "tickers": sorted(affected),
            "deletion_predicate": str(predicate),
        }

    # Purge
    delete_result = await db.stock_prices.delete_many(predicate)

    # Reflag tickers for safe redownload
    reflagged = 0
