    ...
    for attempt in range(max_retries):
        try:
            ...
            await db.ops_job_runs.insert_one({
                ...
                "finished_at_prague": to_prague_iso(completed_at),
                "log_timezone": "Europe/Prague",
                "duration_seconds": (completed_at - started_at).total_seconds(),
                "tickers_targeted": result.get("tickers_targeted", 0) if isinstance(result, dict) else 0,
                "tickers_updated": result.get("tickers_updated", records_processed) if isinstance(result, dict) else records_processed,
                "tickers_skipped_invalid": result.get("tickers_skipped_invalid", 0) if isinstance(result, dict) else 0,
                "tickers_skipped_not_in_universe": result.get("tickers_skipped_not_in_universe", 0) if isinstance(result, dict) else 0,
                "api_calls": result.get("api_calls", 0) if isinstance(result, dict) else 0,
                "api_credits_estimated": result.get("api_credits_estimated", 0) if isinstance(result, dict) else 0,
