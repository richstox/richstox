...
_SAFE_EVENTS_WATERMARK: dict = {"date": None, "days_behind": None, "status": "unknown"}
_SAFE_FUNDAMENTALS_AGE: dict = {
    "total": 0,
    "fresh_7d": {"count": 0, "pct": 0},
    "stale_7_30d": {"count": 0, "pct": 0},
    "stale_30d_plus": {"count": 0, "pct": 0},
    "never_synced": {"count": 0, "pct": 0},
    "oldest": None,
    "newest": None,
}
_SAFE_PENDING_EVENTS: dict = {"count": 0, "oldest_created_at": None, "oldest_ticker": None, "by_type": {}}
...
async def get_data_freshness(db) -> dict:
    ...
    try:
        ...
        return {
            "events_watermark": _SAFE_EVENTS_WATERMARK.copy(),
            "fundamentals_age": _SAFE_FUNDAMENTALS_AGE.copy(),
            "pending_events": _SAFE_PENDING_EVENTS.copy(),
        }
...
async def _get_events_watermark(db) -> dict:
    try:
        ...
        logger.warning("_get_events_watermark failed: %s", exc)
        return _SAFE_EVENTS_WATERMARK.copy()
...
async def _get_fundamentals_age(db) -> dict:
    try:
        ...
        logger.warning("_get_fundamentals_age failed: %s", exc)
        return _SAFE_FUNDAMENTALS_AGE.copy()
...
async def _get_pending_events(db) -> dict:
    try:
        ...
        logger.warning("_get_pending_events failed: %s", exc)
        return _SAFE_PENDING_EVENTS.copy()
