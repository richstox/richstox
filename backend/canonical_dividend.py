...
Source priority: dividend_history FIRST
---------------------------------------------
Primary:   dividend_history collection (EODHD /div/ API — per-share payment records)
Secondary: company_financials.dividends_paid (EODHD Fundamentals cash-flow statement)

Why dividend_history first:
  - Represents actual per-share distributions to shareholders (ex-date + amount)
  - Aligns with what Earnings & Dividends displays — no contradictions
  - company_financials.dividends_paid is a single aggregate cash-flow line item
    with no field-level breakdown; it may include items beyond common-stock dividends

Field-level evidence (data pipeline):
  company_financials.dividends_paid ← EODHD Fundamentals → cf["dividendsPaid"]
  dividend_history.amount           ← EODHD /div/{ticker} API → per-payment records

Integrity rule:
  If BOTH sources produce a yield and they disagree by >20%
  (abs(A-B)/max(abs(A),abs(B)) > 0.20), the result is "unreliable".

Guardrails:
  - >100% yield → extreme_outlier (last-resort safety net, not the core fix)
"""
...
def compute_canonical_dividend_yield(
    *,
    ...
    if hist_yield is not None and cashflow_yield is not None:
        ...
        if dividend_history_count == 0 and cashflow_yield > 0:
            # Cashflow says dividends but dividend_history says none.
            # Two datasets disagree on whether dividends exist at all.
            result["dividend_yield_ttm_value"] = None
            result["source_used"] = "none"
            result["na_reason"] = "unreliable"
            logger.info(
                "Dividend yield mismatch: cashflow_yield=%.2f%% but "
                "dividend_history has 0 records in last 12m",
                cashflow_yield,
            )
        else:
            # dividend_history simply not available (count unknown/0 and yield is 0)
