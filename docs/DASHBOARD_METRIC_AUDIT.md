...
| `history_download_completed` (bool, computed) | `full_sync_service.py` and `scheduler_service.py` (set alongside proof markers at download/remediation time); cleared to `false` by split/dividend detectors | Pre-computed for backward compat. Dashboard facet now derives from proof markers directly. | Legacy backing for **"Complete Prices (strict proof)"** (C8) — dashboard no longer reads this field |
| `gap_free_since_history_download` (bool, computed) | Same writers as above | Pre-computed for backward compat. Dashboard computes gap-free inline from proof markers + bulk coverage. | Legacy backing for **"Gap-Free Since Download"** (C9) — dashboard no longer reads this field |
